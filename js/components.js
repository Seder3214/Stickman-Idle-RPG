...
// data = a function returning the content (actually HTML)
	...
	Vue.component('buyables', {
		...
		<div v-if="tmp[layer].buyables" class="upgTable">
			<respec-button v-if="tmp[layer].buyables.respec && !(tmp[layer].buyables.showRespec !== undefined && tmp[layer].buyables.showRespec == false)" 
			:layer = "layer" v-bind:style="[{'margin-bottom': '12px'}, tmp[layer].componentStyles['respec-button']]"></respec-button>
			<div v-for="row in (data === undefined ? tmp[layer].buyables.rows : data)" class="upgRow">
				<div v-for="col in tmp[layer].buyables.cols"><div v-if="tmp[layer].buyables[row*10+col]!== undefined && tmp[layer].buyables[row*10+col].unlocked" class="upgAlign" 
	...
	Vue.component('buyable', {
		...
		<div v-if="tmp[layer].buyables && tmp[layer].buyables[data]!== undefined && tmp[layer].buyables[data].unlocked" style="display: grid">
			<button v-bind:class="{ buyable: true, tooltipBox: true, can: tmp[layer].buyables[data].canBuy, locked: !tmp[layer].buyables[data].canBuy, 
			...
			<tooltip v-if="tmp[layer].buyables[data].tooltip" v-bind:style='tmp[layer].buyables[data].tooltipStyle' :text="tmp[layer].buyables[data].tooltip"></tooltip>
			...
			<sell-one :layer="layer" :data="data" v-bind:style="tmp[layer].componentStyles['sell-one']" v-if="(tmp[layer].buyables[data].sellOne)&& !(tmp[layer].buyables[data].canSellOne
			!== undefined && tmp[layer].buyables[data].canSellOne == false)"></sell-one>
			<sell-all :layer="layer" :data="data" v-bind:style="tmp[layer].componentStyles['sell-all']" v-if="(tmp[layer].buyables[data].sellAll)&& !(tmp[layer].buyables[data].canSellAll 
			!== undefined && tmp[layer].buyables[data].canSellAll == false)"></sell-all>
		</div>
		`,
	...
	Vue.component('clickable', {
		...
		<button 
			...
			<tooltip v-if="tmp[layer].clickables[data].tooltip" v-bind:style='tmp[layer].clickables[data].tooltipStyle' :text="tmp[layer].clickables[data].tooltip"></tooltip>
	...
	Vue.component('gridable', {
		...
		v-on:click="clickGrid(layer, data)"  @mousedown="start" @mouseleave="stop" @mouseup="stop" @touchstart="start" @touchend="stop" @touchcancel="stop">
			...
			<tooltip v-if="layers[layer].grid.getTooltip" :text="gridRun(this.layer, 'getTooltip', player[this.layer].grid[this.data], this.data)" 
			v-bind:style="gridRun(layer, 'getTooltipStyle', player[this.layer].grid[this.data], this.data)"></tooltip>
...
Vue.component('skill_grid', {
		props: ['layer', 'data'],
		template: `
		<div v-if="tmp[layer].skill_grid" class="upgTable">
			<div v-for="row in (data === undefined ? tmp[layer].skill_grid.rows : data)" class="upgRow">
				<div v-for="col in tmp[layer].skill_grid.cols"><div v-if="run(layers[layer].skill_grid.getUnlocked, layers[layer].skill_grid, row*100+col)"
					class="upgAlign" v-bind:style="{'margin': '1px',  'height': 'inherit',}">
					<skill_gridable :layer = "layer" :data = "row*100+col" v-bind:style="tmp[layer].componentStyles.skill_gridable"></skill_gridable>
				</div></div>
				<br>
			</div>
		</div>
	...
	Vue.component('skill_gridable', {
		...
		<div v-bind:style="skillGridRun(this.layer, 'getProgressStyle', player[this.layer].skill_grid[this.data], this.data)">
		<span v-html="skillGridRun(layer, 'getProgress', player[this.layer].skill_grid[this.data], this.data)"></span>
		</div>
		</button>
		`,
		data() { return { interval: false, time: 0,}},
		computed: {
			canClick() {
				return skillGridRun(this.layer, 'getCanClick', player[this.layer].skill_grid[this.data], this.data)}
		},
		methods: {
			start() {
				if (!this.interval && layers[this.layer].skill_grid.onHold) {
					this.interval = setInterval((function() {
						if(this.time >= 5 && skillGridRun(this.layer, 'getCanClick', player[this.layer].skill_grid[this.data], this.data)) {
							skillGridRun(this.layer, 'onHold', player[this.layer].skill_grid[this.data], this.data)						}	
						this.time = this.time+1
					}).bind(this), 50)}
			},
			stop() {
